...
export default function AttendanceList() {
  ...
  const { data: attendances, isLoading: isLoadingAttendances } = useAttendances(classId, selectedDate);
    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Track daily presence for your students.</p>
        </div>
        
        <div className="flex items-center gap-4">
          <div className="space-y-1">
            <Label htmlFor="date-picker" className="text-xs text-muted-foreground">Select Date</Label>
            <Input 
              id="date-picker"
              type="date" 
              value={selectedDate}
          ...
          <Button onClick={handleSave} disabled={isLoading || saveAttendances.isPending || !students?.length} className="mb-[2px] shadow-md">
            {saveAttendances.isPending ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <Save className="w-4 h-4 mr-2" />}
            Save Attendance
          </Button>
        </div>
      </div>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="text-center py-16 px-4">
              ...
              <p className="text-muted-foreground mt-1">Add students to the class before tracking attendance.</p>
            </div>
          ) : (
            <Table>
                  ...
                  <TableRow key={student.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <TableCell>
                      <RadioGroup 
                        ...
                        onValueChange={(val) => handleStatusChange(student.id, val as Status)}
                      >
                        <div className="flex items-center space-x-2 border rounded-md p-2 cursor-pointer hover:bg-muted transition-colors data-[state=checked]:border-emerald-500 data-[state=checked]:bg-emerald-50">
                          <RadioGroupItem value="PRESENT" id={`present-${student.id}`} className="text-emerald-600 border-emerald-600" />
                          <Label htmlFor={`present-${student.id}`} className="cursor-pointer flex items-center font-medium text-emerald-700">
                        ...
                        <div className="flex items-center space-x-2 border rounded-md p-2 cursor-pointer hover:bg-muted transition-colors data-[state=checked]:border-rose-500 data-[state=checked]:bg-rose-50">
                          <RadioGroupItem value="ABSENT" id={`absent-${student.id}`} className="text-rose-600 border-rose-600" />
                          <Label htmlFor={`absent-${student.id}`} className="cursor-pointer flex items-center font-medium text-rose-700">
                        ...
                        <div className="flex items-center space-x-2 border rounded-md p-2 cursor-pointer hover:bg-muted transition-colors data-[state=checked]:border-amber-500 data-[state=checked]:bg-amber-50">
                          <RadioGroupItem value="LATE" id={`late-${student.id}`} className="text-amber-600 border-amber-600" />
                          <Label htmlFor={`late-${student.id}`} className="cursor-pointer flex items-center font-medium text-amber-700">
